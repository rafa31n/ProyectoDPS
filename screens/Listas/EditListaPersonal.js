...
const EditListaPersonal = () => {
    ...
    useEffect(() => {
        AsyncStorage.getItem('listaPersonalEdit')
            .then((data) => {
                if (data) {
                    const datos = JSON.parse(data);
                    setIdLista(datos)
                    console.log(datos)
                    fetch("http://10.0.2.2:4000/api/lista_personal/uno/" + datos)
                        .then((response) => response.json())
                        .then((data) => {
                            setTitulo(data.body[0].titulo)
                            setDescripcion(data.body[0].descripcion)
                        })
                        .catch((error) => {
            ...
            <ScrollView>
                <View style={styles.headerContainer}>
                    ...
                    <View style={styles.centerElement}>
                        <Text style={styles.header}>Editar lista personal</Text>
                    </View>
                </View>
...
export default EditListaPersonal;